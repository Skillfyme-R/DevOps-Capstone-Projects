...
export default function AnalyticsPage() {
  const { data: summary } = useQuery('analytics-summary', () => analyticsClient.get('/analytics/summary').then((r: { data: any }) => r.data), { retry: false });
  const { data: trend } = useQuery('analytics-trend', () => analyticsClient.get('/analytics/appointments/trend?days=30').then((r: { data: any }) => r.data), { retry: false });
  const { data: kpis } = useQuery('analytics-kpis', () => analyticsClient.get('/analytics/operational/kpis').then((r: { data: any }) => r.data), { retry: false });
  const { data: conditions } = useQuery('analytics-conditions', () => analyticsClient.get('/analytics/clinical/conditions').then((r: { data: any }) => r.data), { retry: false });
  const { data: demographics } = useQuery('analytics-demographics', () => analyticsClient.get('/analytics/patients/demographics').then((r: { data: any }) => r.data), { retry: false });
